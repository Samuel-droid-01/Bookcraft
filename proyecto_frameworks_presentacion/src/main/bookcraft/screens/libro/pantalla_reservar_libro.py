...
from proyecto_frameworks_persistencia.src.main.bookcraft.dao.registro.registroDAO import RegistroDAO
from datetime import datetime
from tkinter import *
from tkinter import messagebox as ms

class PantallaReservarLibro:
    ...
    def reservar(self, idLibro, idUsuario):
        registroDao=RegistroDAO()
        fecha_actual = datetime.now().strftime("%Y-%m-%d")
        print(f"Reservando libro {idLibro} para usuario {idUsuario} en fecha {fecha_actual}")
        registroDao.reservar_prestamo(idLibro, idUsuario, fecha_actual)
        ms.showinfo("Reservar", "Libro reservado con éxito")
    ...
    def mostrar_libro(self, libro):
        ...
        btnDevolver = Button(frame_libro, padx=10, pady=5, bd=5, font=('arial', 9, 'bold'), text='Reservar', bg="#2E4053", fg="white", command=lambda idLibro=libro.get_id(), idUsuario=self.id_usuario: self.reservar(idLibro, idUsuario))
